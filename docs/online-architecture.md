...
サーバも Unity クライアントも同じ `SkullDive.Net.WireCodec` を使う。
リフレクションを使わず、フィールドを 1 つずつ手で読み書きしている。理由は 3 つ:

- **Unity にパッケージを追加させない。** セットアップの手間とバージョン依存が消える
- **IL2CPP のマネージドコード除去に強い。** リフレクション経由でしか触られない DTO の
  フィールドは実機ビルドで削られることがあり、「エディタでは動くが実機で中身が空になる」
  という形で壊れる。手書きなら参照が静的に残るのでこれが起きない
- **実装が 1 つなので書式のズレが原理的に起きない。** サーバとクライアントで別の
  シリアライザを使うと、enum の表現(数値か文字列か)やフィールドの扱いが食い違う余地が残る

手書きである分の検証として、標準的なシリアライザ (System.Text.Json) を独立したオラクルに
使い、双方向の相互運用をテストで固定している:

- `WireCodec` が出した JSON を `System.Text.Json` が読めること
- `System.Text.Json` が出した JSON を `WireCodec` が読めること

これによりキー名も値の表現も標準から外れていないことが保証され、
デバッグ時に汎用ツールで通信内容を読める。
